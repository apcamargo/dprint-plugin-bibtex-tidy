var BibTeXTidyModule = (function(exports) {
	Object.defineProperty(exports, Symbol.toStringTag, { value: "Module" });
	//#region node_modules/bibtex-tidy/bibtex-tidy.js
	/**
	* bibtex-tidy v1.15.1
	...
	**/
	...
	var BlockNode = class _BlockNode extends BaseNode {
		constructor(kind, parent, children = []) {
			...
			if (parent instanceof _BlockNode) parent.children.push(this);
			else if (parent instanceof CommandNode) parent.args.push(this);
		}
		static {
	...
	function parseLaTeX(input) {
		...
		for (let i = 0; i < input.length; i++) {
			...
			switch (node.type) {
				case "block":
					if (char === "\\") node = new CommandNode(node);
					else if (char === "$") {
						const math = parseMath(input, i);
						if (math) {
							new MathNode(node, math.text);
							i = math.end;
						} else node = new TextNode(node, char);
					} else if (char === "{") node = new BlockNode("curly", node);
					else if ((char === "}" && node.kind === "curly" || char === "]" && node.kind === "square") && node.parent) node = node.parent;
					else node = new TextNode(node, char);
					break;
				case "text":
					if (char === "$") {
					...
					break;
				case "command": if (char === "{") node = new BlockNode("curly", node);
				else if (char === "[") node = new BlockNode("square", node);
				else if (char === "}" && node.parent.kind === "curly" || char === "]" && node.parent.kind === "square" || /\s/.test(char) || node.args.length > 0) {
					node = node.parent;
					i--;
				} else node.command += char;
			}
		}
		return rootNode;
	}
	...
	function stringifyBlock(block) {
		const content = block.children.map((node) => {
			switch (node.type) {
				case "block": return stringifyBlock(node);
				case "command": return stringifyCommand(node);
				case "math": return `$${node.text}$`;
				case "text": return node.text;
				default: throw new Error(`Unknown node type: ${JSON.stringify(node)}`);
			}
		}).join("");
		switch (block.kind) {
			case "root": return content;
			case "curly": return `{${content}}`;
			case "square": return `[${content}]`;
		}
	}
	...
	function flattenLaTeX(block) {
		...
		for (const child of block.children) if (child.type === "block" && child.kind === "curly" && child.children.every((child2) => child2.type !== "command")) {
			const newChild = flattenLaTeX(child);
			newBlock.children.push(...newChild.children);
		} else newBlock.children.push(child);
		return newBlock;
	}
	...
	function parseBibTeX(input) {
		...
		for (let i = 0; i < input.length; i++) {
			...
			switch (node.type) {
				case "root":
					if (char === "@") node = new BlockNode2(node, flushWhitespace());
					else if (isWhitespace(char)) whitespace += char;
					else node = new TextNode2(node, char, flushWhitespace());
					break;
				case "text":
					if (char === "@" && /[\s\r\n}]/.test(prev)) node = new BlockNode2(node.parent, "");
					else node.text += char;
					break;
				case "block":
					if (char === "@") {
						const prevNode = node.parent.children[node.parent.children.length - 2];
						if (isNodeType(prevNode, "text")) prevNode.text += `@${node.command}`;
						else {
							node.parent.children.pop();
							new TextNode2(node.parent, `@${node.command}`, node.whitespacePrefix);
						...
						if (commandTrimmed === "" || /\s/.test(commandTrimmed)) {
							node.parent.children.pop();
							node = new TextNode2(node.parent, `@${node.command}${char}`, node.whitespacePrefix);
						} else {
							...
							switch (command) {
								...
								case "comment":
									...
									break;
								default: node = new EntryNode(node, char);
							}
						}
					} else if (char.match(/[=#,})[\]]/)) {
						node.parent.children.pop();
						node = new TextNode2(node.parent, `@${node.command}${char}`, flushWhitespace());
					} else node.command += char;
					break;
				case "comment":
				case "string":
				case "preamble":
					if (char === "{") node.braces++;
					else if (char === "}") node.braces--;
					else if (char === "(") node.parens++;
					else if (char === ")") node.parens--;
					node.raw += char;
					if (node.braces === 0 && node.parens === 0) node = node.parent.parent;
					break;
				case "entry":
					if (isWhitespace(char)) {
						if (!node.key) {} else node.keyEnded = true;
					} else if (char === ",") node = new FieldNode(node);
					else if (node.wrapType === "{" && char === "}" || node.wrapType === "(" && char === ")") node = node.parent.parent;
					else if (char === "=" && node.key && isValidFieldName(node.key)) {
						const field = new FieldNode(node, node.key);
						node.fields.push(field);
						node.key = void 0;
						node = field.value;
					} else if (node.keyEnded) throw new BibTeXSyntaxError(input, node, i, line, column, "The entry key cannot contain whitespace");
					else if (!isValidKeyCharacter(char)) throw new BibTeXSyntaxError(input, node, i, line, column, `The entry key cannot contain the character (${char})`);
					else node.key = (node.key ?? "") + char;
					break;
				case "field":
					if (char === "}" || char === ")") {
					...
					} else if (!isValidFieldName(char)) throw new BibTeXSyntaxError(input, node, i, line, column);
					else if (!node.name) {
						if (!isWhitespace(char)) {
							node.parent.fields.push(node);
							node.name = char;
						} else node.whitespacePrefix += char;
					} else node.name += char;
					break;
				case "concat":
					if (isWhitespace(char)) break;
					if (node.canConsumeValue) {
						if (/[#=,}()[\]]/.test(char)) throw new BibTeXSyntaxError(input, node, i, line, column);
						...
						} else node = createLiteralNode(node, char);
					} else if (char === ",") node = new FieldNode(node.parent.parent);
					else if (char === "}" || char === ")") node = node.parent.parent.parent.parent;
					else if (char === "#") node.canConsumeValue = true;
					else throw new BibTeXSyntaxError(input, node, i, line, column);
					break;
				case "literal":
					if (isWhitespace(char)) node = node.parent;
					else if (char === ",") node = new FieldNode(node.parent.parent.parent);
					else if (char === "}") node = node.parent.parent.parent.parent.parent;
					else if (char === "#") {
						node = node.parent;
				...
				case "quoted":
					...
					if (char === "{") node.depth++;
						...
						node.depth--;
						if (node.depth < 0) throw new BibTeXSyntaxError(input, node, i, line, column);
					}
					latexInput += char;
			}
		}
	...
	var BibTeXSyntaxError = class extends Error {
		...
		constructor(input, node, pos, line, column, hint) {
			super(`Line ${line}:${column}: Syntax Error in ${node.type} (${hint})
${input.slice(Math.max(0, pos - 20), pos)}>>${input[pos]}<<${input.slice(pos + 1, pos + 20)}`);
	...
	function getField(entry, fieldName) {
		...
		return entry.fields.find((field) => field.name.toLocaleLowerCase() === fieldNameLc);
	}
	...
	var ASTProxy = class {
		...
		replace(where, enter) {
			const replaceNodes = /* @__PURE__ */ __name((nodes, index, replacements, parent) => {
				...
				for (const replacement of replacements) setParent(replacement, parent);
				return index;
			}, "replaceNodes");
			...
			const visitReplaceable = /* @__PURE__ */ __name((nodes, index, ancestors, parent) => {
				...
				if (replacements) {
					const replacementIndex = replaceNodes(nodes, index, replacements, parent);
					for (const replacement of replacements) visitChildren(replacement, [...ancestors, replacement]);
			...
			const visitConcat = /* @__PURE__ */ __name((node, ancestors, parent) => {
				...
				if (replacements.length !== 1) throw new Error("A concat node must be replaced by exactly one node");
			...
			const visitChildren = /* @__PURE__ */ __name((node, ancestors) => {
				switch (node.type) {
					case "root":
						for (let i = 0; i < node.children.length;) i = visitReplaceable(node.children, i, ancestors, node);
						break;
					case "block":
						if (node.block) visitChildren(node.block, [...ancestors, node.block]);
						break;
					case "entry":
						for (let i = 0; i < node.fields.length;) i = visitReplaceable(node.fields, i, ancestors, node);
						break;
					case "field":
						visitConcat(node.value, ancestors, node);
						break;
					case "concat": for (let i = 0; i < node.concat.length;) i = visitReplaceable(node.concat, i, ancestors, node);
				}
			}, "visitChildren");
		...
		switch (node.type) {
			...
			case "block":
				...
				if (isNodeType(node, "block") && node.block) {
					...
					if (isNodeType(node.block, "entry")) for (const field of node.block.fields) setParent(field, node.block);
				}
				break;
			case "field":
			...
			case "concat":
				...
				for (const child of node.concat) child.parent = node;
				break;
			case "literal":
			case "braced":
			case "quoted": if (isNodeType(parent, "concat")) node.parent = parent;
		}
	}
	__name(setParent, "setParent");
	function logAST(node, depth = 0) {
		let log = `
${"  ".repeat(depth)}${node.type} ws="${"whitespacePrefix" in node ? node.whitespacePrefix.replace(/\n/g, "\\n") : ""}"`;
		switch (node.type) {
			case "root":
				for (const child of node.children) log += logAST(child, depth + 1);
				break;
			case "block":
				log += ` command="${node.command}"`;
				if (node.block) log += logAST(node.block, depth + 1);
				break;
			case "concat":
				for (const value of node.concat) log += logAST(value, depth + 1);
				break;
			case "entry":
				log += ` key="${node.key}"`;
				for (const field of node.fields) log += logAST(field, depth + 1);
				break;
			case "field":
				log += ` name="${node.name}"`;
				log += logAST(node.value, depth + 1);
				break;
			case "text": log += ` text="${node.text.replace(/\n/g, "\\n")}"`;
		}
		return log;
	}
	...
	function replaceValueNodeText(node, value) {
		if (isNodeType(node, "literal")) return node.with({ value });
	...
	function encloseLatexInCurly(latex) {
		if (latex.children.length === 1 && latex.children[0]?.type === "block" && latex.children[0]?.kind === "curly" && latex.children[0].children.length === 1 && latex.children[0].children[0]?.type === "block" && latex.children[0].children[0]?.kind === "curly") return latex;
	...
	function formatBibtex(ast) {
		return `${ast.children.map((child) => formatNode(child)).join("").trimEnd()}
`;
	}
	__name(formatBibtex, "formatBibtex");
	function formatNode(child) {
		if (isNodeType(child, "text")) return `${child.whitespacePrefix}${child.text}`;
		if (!child.block) throw new Error("FATAL!");
		switch (child.block.type) {
			case "preamble":
			case "string":
			case "comment": return `${child.whitespacePrefix}${child.block.raw}`;
			case "entry": return `${child.whitespacePrefix}${formatEntry(child.command, child.block)}`;
		}
	}
	...
	function formatEntry(entryType, entry) {
		...
		for (const field of entry.fields) {
			...
			if (value) bibtex += `${field.value.whitespacePrefix}= ${value}`;
	...
	function formatValue(field) {
		return field.value.concat.map((node) => {
			switch (node.type) {
				...
				case "braced": return doubleEncloseLatex(node.latexAst);
				case "quoted": return `"${renderValueNode(node)}"`;
				default: throw new Error(`Unknown value type: ${JSON.stringify(node)}`);
			}
		}).join(" # ");
	}
	__name(formatValue, "formatValue");
	var DEFAULT_MERGE_CHECK = [
		"doi",
		"citation",
		"abstract"
	];
	...
	var optionDefinitions = [
		...
		{
			key: "space",
			cli: { "--space": /* @__PURE__ */ __name((args) => args.length > 0 ? Number(args[0]) : true, "--space") },
			toCLI: /* @__PURE__ */ __name((val, opt) => {
				if (opt.tab) return void 0;
				if (typeof val === "number" && val !== DEFAULT_SPACE) return `--space=${val}`;
		...
		},
		{
			...
			toCLI: /* @__PURE__ */ __name((val) => {
				...
				if (typeof val === "number" && val !== DEFAULT_ALIGN) return `--align=${val}`;
			}, "toCLI"),
			title: "Align values",
		...
		{
			key: "duplicates",
			cli: { "--duplicates": /* @__PURE__ */ __name((args) => {
				if (args.length === 0) return true;
				for (const i of args) if (i !== "doi" && i !== "key" && i !== "abstract" && i !== "citation") {
		...
		},
		{
			...
			description: ["Escape special characters, such as umlauts. This is usually unnecessary for modern toolchains like biber and biblatex. Enabled by default.", "By default, the legacy escape list is used for v1 compatibility and may emit macros requiring external packages. Use --escape=new to only convert characters with package-independent LaTeX escapes and warn on characters that cannot be escaped. New mode will become the default in v2; please raise a GitHub issue if you still need the legacy behavior."],
			type: "boolean | 'new'",
			defaultValue: true
		},
		{
		...
		},
		{
			...
			toCLI: /* @__PURE__ */ __name((val) => {
				if (Array.isArray(val) && val.length > 0) {
					if (JSON.stringify(val) === JSON.stringify(DEFAULT_FIELD_SORT)) return "--sort-fields";
		...
		},
		{
			...
			toCLI: /* @__PURE__ */ __name((val) => {
				...
				if (typeof val === "string") return `--generate-keys="${val.replace(/"/g, "\\\"")}"`;
			}, "toCLI"),
		...
		{
			key: "enclosingBraces",
			cli: { "--enclosing-braces": /* @__PURE__ */ __name((args) => args.length > 0 ? args : true, "--enclosing-braces") },
			toCLI: /* @__PURE__ */ __name((val) => {
		...
		{
			key: "removeBraces",
			cli: { "--remove-braces": /* @__PURE__ */ __name((args) => args.length > 0 ? args : true, "--remove-braces") },
			toCLI: /* @__PURE__ */ __name((val) => {
				if (Array.isArray(val) && val.length > 0) return `--remove-braces=${val.join(",")}`;
	...
	function normalizeOptions(options) {
		return Object.fromEntries(optionDefinitions.map((def) => {
			...
			if (def.convertBoolean && typeof value === "boolean") return [key, value ? def.convertBoolean.true : def.convertBoolean.false];
			if (typeof value === "undefined" && def.defaultValue !== void 0) {
				if (typeof def.defaultValue === "function") return [key, def.defaultValue(options)];
	...
	function createAbbreviateMonthsTransform() {
		const months = new Map(Object.entries(monthAliases).flatMap(([abbr, aliases]) => aliases.map((alias) => [alias, abbr])));
		return {
			name: "abbreviate-months",
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node, ctx) => isNodeType(node, "literal", "braced", "quoted") && ctx.closestAncestor("field")?.name.toLowerCase() === "month", (node) => {
	...
	function createAlignValuesTransform(column) {
		return {
			name: "align-values",
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node) => isNodeType(node, "concat"), (concat) => [concat.with({ whitespacePrefix: " ".repeat(Math.max(column - concat.parent.name.length, 1)) })]);
			}, "apply")
		};
	}
	...
	function createBlankLinesTransform() {
		return {
			name: "blank-lines",
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node) => isNodeType(node, "text", "block"), (child) => {
					const index = child.parent.children.indexOf(child);
					const prev = child.parent.children[index - 1];
					return [child.with({ whitespacePrefix: prev && !isComment(prev) ? "\n\n" : child.whitespacePrefix })];
				});
			}, "apply")
		};
	}
	...
	function renderFieldValue(field) {
		return field.value.concat.map((node) => isNodeType(node, "literal") ? node.value : node.latexAst.renderAsText()).join(" # ");
	}
	...
	function createDropAllCapsTransform() {
		return {
			...
			apply: /* @__PURE__ */ __name((ast) => {
				...
				}, (node) => [replaceValueNodeText(node, titleCase(renderValueNode(node)))]);
			}, "apply")
		};
	}
	...
	function createEncodeUrlsTransform() {
		return {
			name: "encode-urls",
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node, ctx) => isNodeType(node, "braced", "quoted") && ctx.closestAncestor("field")?.name.toLocaleLowerCase() === "url", (entry) => [replaceValueNodeText(entry, encodeUrl(renderValueNode(entry)))]);
			}, "apply")
		};
	}
		...
		return {
			...
			apply: /* @__PURE__ */ __name((ast) => {
				...
				ast.replace((node, ctx) => {
					...
					const result = escapeChars(renderValueNode(node), chars, isNodeType(node, "quoted"));
					for (const unsupported of result.unsupported) warnings.set(`${field.name}:${unsupported.codepoint}`, {
	...
	function escapeChars(value, characters, protectQuotes = false) {
		...
		for (const char of result) {
			...
			if (!escaped && codepointValue > 127) unsupported.push({
				character: char,
				codepoint
			});
		}
		return {
			value: newstr.replace(/MATH\.EXP\.(\d+)/g, (_, i) => mathExpressions[Number(i)] ?? ""),
			unsupported
		};
	}
	...
	function createFieldCommasTransform(trailing) {
		return {
			name: "field-commas",
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node) => isNodeType(node, "field"), (field) => [field.with({ hasComma: field.parent.fields.indexOf(field) < field.parent.fields.length - 1 || trailing })]);
			}, "apply")
		};
	}
	...
	function createFormatPageRangeTransform() {
		return {
			...
			apply(ast) {
				ast.replace((node, ctx) => isNodeType(node, "braced", "quoted") && ctx.closestAncestor("field")?.name.toLocaleLowerCase() === "pages", (entry) => [replaceValueNodeText(entry, formatPageRange(renderValueNode(entry)))]);
			}
		};
	}
	...
	function formatPageRange(str) {
		let result = str;
		for (let i = 0; i < 4; i++) result = result.replace(/(\d)\s*-\s*(\d)/g, "$1--$2");
		return result;
	}
	...
	function parseEntryKeyTemplate(template) {
		...
		for (const match of matches) {
			...
			if (match.index !== pos) tokens.push(template.slice(pos, match.index));
			const [tokenKeyN, ...modifierKeys] = match[0].slice(1, -1).split(":");
			if (!tokenKeyN) throw new Error("Token parse error");
			let n;
		...
		if (pos < template.length) tokens.push(template.slice(pos));
		return tokens;
	}
	...
	function detectNameSyntax(tokens) {
		...
		if (tokens.length === 1 && nameStr(tokens) === "others") return "Others";
		if (tokens.length === names.length && tokens.length === 1) return "LastName";
		if (tokens.length === names.length) return "FirstName LastNames";
		if (prefixes.length > 0 && commas.length === 0) return "FirstNames Prefixes LastNames";
		if (commas.length === 1) return "LastNames, FirstNames Prefixes";
		if (commas.length === 2) return "LastNames, Suffixes, FirstNames Prefixes";
		throw new Error(`Invalid name syntax: ${tokens.map((token) => token.type).join(" ")}`);
	}
	...
	function nameStr(tokens) {
		return tokens.filter((token) => token.type !== "comma").map((token) => token.value).join(" ");
	}
	...
	function partition(tokens, divideBefore) {
		...
		for (const token of tokens) {
			if (divideBefore[currPartition + 1] === token.type) currPartition++;
	...
	var SPECIAL_MARKERS = {
		auth: {
			...
			callback: /* @__PURE__ */ __name((v) => {
				const author = parseNameList(v.get("author") ?? "")[0]?.last;
		...
		authEtAl: {
			...
			callback: /* @__PURE__ */ __name((v) => {
				...
				return [...authors.slice(0, 2).map((author) => author.last), ...authors.length > 2 ? ["Et", "Al"] : []];
			}, "callback")
		},
		authors: {
			description: "Last name all authors",
			callback: /* @__PURE__ */ __name((v) => {
				return parseNameList(v.get("author") ?? "").map((author) => author.last);
			}, "callback")
		},
		authorsN: {
			...
			callback: /* @__PURE__ */ __name((v, n = 0) => {
				const authors = parseNameList(v.get("author") ?? "");
				return [...authors.slice(0, n).map((author) => author.last), ...authors.length > n ? ["Et", "Al"] : []];
			}, "callback")
		},
		veryshorttitle: {
	...
	function generateKeys(entries, entryKeyTemplate) {
		...
		if (!entryKeyTemplate.includes("[duplicateLetter]") && !entryKeyTemplate.includes("[duplicateNumber]")) template = `${entryKeyTemplate}[duplicateLetter]`;
		...
		for (const entry of entries) {
			const entryValues = new Map(entry.fields.map((field) => [field.name, renderFieldValue(field)]));
	...
	function generateKey(valueLookup, entryKeyTemplate, duplicateNumber) {
		try {
			let newKey = entryKeyTemplate.map((token) => {
				...
				let key;
				if (specialMarker) key = specialMarker.callback(valueLookup, parameter, duplicateNumber);
				else if (marker === marker.toLocaleUpperCase()) {
					const value = valueLookup.get(marker.toLocaleLowerCase());
					key = value ? words(value) : [];
				} else throw new Error(`Invalid citation key token ${marker}`);
				for (const modifierKey of modifiers) {
					const modifier = MODIFIERS[modifierKey];
					if (modifier) key = modifier.callback(key);
					else throw new Error(`Invalid modifier ${modifierKey}`);
				}
				return key.join("");
			}).join("");
			...
			if (e instanceof MissingRequiredData) return;
			throw e;
		}
	}
	...
	function createGenerateKeysTransform(template) {
		return {
			...
			apply: /* @__PURE__ */ __name((ast) => {
				...
				ast.replace((node) => isNodeType(node, "block") && isNodeType(node.block, "entry"), (node) => {
	...
	function createIndentFieldsTransform(indent) {
		return {
			name: "indent",
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node) => isNodeType(node, "field"), (field) => [field.with({ whitespacePrefix: `
${indent}` })]);
			}, "apply")
		};
	}
	...
	function createLimitAuthorsTransform(maxAuthors) {
		return {
			name: "limit-authors",
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node, ctx) => isNodeType(node, "braced", "quoted") && ctx.closestAncestor("field")?.name.toLocaleLowerCase() === "author", (node) => {
					const authors = renderValueNode(node).split(" and ");
					if (authors.length > maxAuthors) return [replaceValueNodeText(node, [...authors.slice(0, maxAuthors), "others"].join(" and "))];
					return [node];
				});
			}, "apply")
		};
	...
	function createLowercaseEntryTypeTransform() {
		return {
			...
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node) => isNodeType(node, "block") && isNodeType(node.block, "entry"), (node) => [node.with({ command: node.command.toLocaleLowerCase() })]);
			}, "apply")
		};
	}
	...
	function createLowercaseFieldsTransform() {
		return {
			...
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node) => isNodeType(node, "field"), (field) => [field.with({ name: field.name.toLocaleLowerCase() })]);
			}, "apply")
		};
	}
	...
	function checkForDuplicates(ast, duplicateRules, merge) {
		...
		if (duplicateRules) for (const rule of duplicateRules) rules.set(rule, !!merge);
		if (!rules.has("key")) rules.set("key", false);
		...
		for (const entry of ast.entries()) for (const [rule, doMerge] of rules) {
			let duplicateOf;
			let warning;
			switch (rule) {
				case "key": {
					...
					else warning = `The citation key ${entry.key} has already been used.`;
					break;
				}
				case "doi": {
					...
					else warning = `Entry ${entry.key} has an identical DOI to entry ${duplicateOf.key}.`;
					break;
				}
				case "citation": {
					...
					else warning = `Entry ${entry.key} has similar content to entry ${duplicateOf.key}.`;
					break;
				}
				case "abstract": {
					const field = getField(entry, "abstract");
					const abs = alphaNum(field ? renderFieldValue(field) : "").slice(0, 100);
					if (!abs) continue;
					duplicateOf = abstracts.get(abs);
					if (!duplicateOf) abstracts.set(abs, entry);
					else warning = `Entry ${entry.key} has a similar abstract to entry ${duplicateOf.key}.`;
					break;
				}
			}
		...
		switch (merge) {
			...
			case "overwrite":
				for (const field of entry.fields) {
					...
					if (!existing) duplicateOf.fields.push(field);
					else if (merge === "overwrite") existing.value = field.value;
				}
				break;
			case "first": return;
		}
	...
	function createMergeEntriesTransform(duplicatesOpt, merge) {
		return {
			...
			apply: /* @__PURE__ */ __name((ast) => {
				...
				ast.replace((node) => isNodeType(node, "block") && isNodeType(node.block, "entry") && duplicates.entries.has(node.block), () => []);
	...
	function createPreferCurlyTransform() {
		return {
			...
			apply: /* @__PURE__ */ __name((ast) => {
				...
				}, (child) => [new BracedNode(child.parent).with({ latexAst: isNodeType(child, "quoted") ? child.latexAst : createTextAst(child.value) })]);
			}, "apply")
		};
	}
	...
	function createPreferNumericTransform() {
		return {
			...
			dependencies: ["prefer-curly"],
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node) => isNodeType(node, "braced", "quoted") && /^[1-9][0-9]*$/.test(renderValueNode(node)), (node) => [new LiteralNode(node.parent, renderValueNode(node))]);
			}, "apply")
		};
	}
	...
	function createRemoveCommentsTransform() {
		return {
			...
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node) => isNodeType(node, "text") || isNodeType(node, "block") && isNodeType(node.block, "comment"), () => []);
			}, "apply")
		};
	}
	...
	function createRemoveDuplicateFieldsTransform() {
		return {
			...
			apply: /* @__PURE__ */ __name((ast) => {
				...
				ast.replace((node) => isNodeType(node, "field"), (field) => {
					const seenFields = seenFieldsByEntry.getOrInsert(field.parent, /* @__PURE__ */ new Set());
	...
	function createRemoveEmptyFieldsTransform() {
		return {
			...
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node) => isNodeType(node, "field") && !node.value.concat.some((node2) => renderValueNode(node2).trim() !== ""), () => []);
			}, "apply")
		};
	}
	...
	function createRemoveEnclosingBracesTransform() {
		return {
			...
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node) => isNodeType(node, "braced"), (node) => [replaceValueNodeText(node, renderValueNode(node).replace(/^\{([^{}]*)\}$/g, "$1"))]);
			}, "apply")
		};
	}
	...
	function createRemoveSpecifiedFieldsTransform(omit) {
		return {
			...
			apply(ast) {
				...
				ast.replace((node) => isNodeType(node, "field") && set.has(node.name.toLocaleLowerCase()), () => []);
			}
		};
	}
...
`;
					if (!preserve) {
						...
						if (isNodeType(child, "text")) child.text = child.text.trim();
						else if (child.block) {
							if (isNodeType(child.block, "entry")) for (const field of child.block.fields) field.whitespacePrefix = "";
							else if (isNodeType(child.block, "comment")) child.block.raw = child.block.raw.trim();
						}
					}
					prev = child;
					return [child];
	...
	function sortEntries(root, sort) {
		...
		const sortableChildren = root.children.filter((item) => !fixedText.has(item));
	...
	function createSortFieldsTransform(sortFields) {
		return {
			name: "sort-fields",
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node) => isNodeType(node, "block") && isNodeType(node.block, "entry"), (node) => [node.with({ block: sortEntryFields(node.block, sortFields) })]);
			}, "apply")
		};
	}
	...
	var coreQuotedUnescapeCharacters = coreUnescapeCharacters.filter(({ escaped }) => escaped.includes("\"")).map(({ character, escaped }) => ({
		character,
		escaped: `{${escaped}}`
	}));
	function unescapeCharacters(value, quoted = false) {
		let result = value;
		if (quoted) for (const { escaped, character } of coreQuotedUnescapeCharacters) result = result.replaceAll(escaped, character);
		for (const { escaped, character } of coreUnescapeCharacters) result = result.replaceAll(escaped, character);
		return result;
	}
	...
	function createUnescapeCharactersTransform() {
		return {
			...
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node) => isNodeType(node, "braced", "quoted"), (node) => [replaceValueNodeText(node, unescapeCharacters(renderValueNode(node), isNodeType(node, "quoted")))]);
			}, "apply")
		};
	}
	...
	function createWrapValuesTransform(indent, align, wrap) {
		return {
			name: "wrap-values",
			apply: /* @__PURE__ */ __name((ast) => {
				ast.replace((node) => isNodeType(node, "braced"), (node) => {
					let value = unwrapText(renderValueNode(node));
					if (node.parent.concat.length === 1) value = value.trim();
...
${indent}`;
	...
	function generateTransformPipeline(options) {
		const pipeline = [];
		if (options.months) pipeline.push(createAbbreviateMonthsTransform());
		if (options.dropAllCaps) pipeline.push(createDropAllCapsTransform());
		if (options.encodeUrls) pipeline.push(createEncodeUrlsTransform());
		if (options.unescape) pipeline.push(createUnescapeCharactersTransform());
		else if (options.escape) pipeline.push(createEscapeCharactersTransform(options.escape === "new"));
		pipeline.push(createFormatPageRangeTransform());
		if (options.generateKeys) pipeline.push(createGenerateKeysTransform(options.generateKeys));
		if (options.maxAuthors) pipeline.push(createLimitAuthorsTransform(options.maxAuthors));
		if (options.lowercase) pipeline.push(createLowercaseEntryTypeTransform(), createLowercaseFieldsTransform());
		if (options.merge || options.duplicates) pipeline.push(createMergeEntriesTransform(options.duplicates, options.merge));
		if (options.omit) pipeline.push(createRemoveSpecifiedFieldsTransform(options.omit));
		if (options.enclosingBraces) pipeline.push(createEncloseBracesTransform(options.enclosingBraces));
		if (options.curly) pipeline.push(createPreferCurlyTransform());
		if (options.numeric) pipeline.push(createPreferNumericTransform());
		if (options.removeBraces) pipeline.push(createRemoveBracesTransform(options.removeBraces));
		if (options.stripComments) pipeline.push(createRemoveCommentsTransform());
		if (options.removeDuplicateFields) pipeline.push(createRemoveDuplicateFieldsTransform());
		if (options.removeEmptyFields) pipeline.push(createRemoveEmptyFieldsTransform());
		if (options.sort) pipeline.push(createSortEntriesTransform(options.sort));
		if (options.sortFields) pipeline.push(createSortFieldsTransform(options.sortFields));
		if (options.stripEnclosingBraces) pipeline.push(createRemoveEnclosingBracesTransform());
		...
		if (options.blankLines) pipeline.push(createBlankLinesTransform());
	...
	function tidy(input, options_ = {}) {
		...
		const ast = parseBibTeX(convertCRLF(input));
		...
		if (verbose) console.log(logAST(ast));
